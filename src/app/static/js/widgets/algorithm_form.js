...
class AlgorithmFormManager {
	...
	currentAlgorithm(div = null) {
		...
		if (currentAlgorithm) {
			...
			let currentConfig = utils.toJson(
				$(`#${div.id} :selected`).attr(sel.DATA.algorithmObject),
			);
		...
		this.getFormsetItem(formTitle).each((i, item) => {
			$(item)
				...
				.on("click", () => {
					...
					this.getTotalFormsInput(formTitle).val(
						this.getFormsetItem(formTitle).length,
					);
	...
	addFormsetItem(formTitle) {
		...
		const formsetItemIndex = this.getTotalFormsInput(formTitle).val();
		// Clone the form
		...
		this.getTotalFormsInput(formTitle).val(parseInt(formsetItemIndex) + 1);
		this.updateFormIndices(formTitle);
	...
	populateFormset(formTitle, data, populateForm) {
		...
		this.getTotalFormsInput(formTitle).val(data.length);
	}
	...
	checkEditAlgorithm(button) {
		if (button.hasClass("edit-algorithm")) {
			let algorithmId = button
				.closest(sel.CLASS.algorithm)
				.attr(sel.DATA.algorithmId);
			$("#dropdown2").val(algorithmId).trigger("change");
		}
	}
