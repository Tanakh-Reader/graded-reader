...
export class CompareWidgetsMode {
	...
	setDeleteButtonListeners() {
		$(compareWidgetsDiv)
			...
			.on("click", (event) => {
				...
				if (this.passageWidgets().length > constants.minTextWidgets) {
					event.currentTarget.closest(".passage-widget").remove();
				} else {
					utils.showToast(
						`Minimum of ${constants.minTextWidgets} text widgets.`,
						2000,
					);
				}
			});
	...
	addTextWidget() {
		...
		if (passages.length < constants.maxTextWidgets) {
			...
			utils.showToast(
				`Maximum of ${constants.maxTextWidgets} text widgets.`,
				2000,
			);
		}
	}
