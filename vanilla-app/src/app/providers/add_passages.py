...
def valid_passage(
    passage: Passage, verse: int, passage_size_min: int, passage_size_max: int
) -> tuple[bool, bool]:
    is_valid = False
    add_verse = True
...
def get_passage_tags(passage: Passage) -> str:
    tags = ""
    word_count = 0
    proper_noun_count = 0
    has_qere = False
    ...
    for word in passage.word_ids():
        ...
        if hdp.lex_stripped(word) not in Classify().stop_words:
            ...
            if is_proper_noun(word):
                proper_noun_count += 1
            # Check for qere
            if not has_qere and hdp.qere(word) not in ["", None]:
                has_qere = True
                tags += f"{TAGS.QERE},"
            word_count += 1
...
def update_passage_data(passage: Passage):
    ...
    passage.word_count = len(passage.word_ids())
...
def get_passages(
    ...
    passage_size_min: int = 100,
    passage_size_max: int = 4000,
) -> list[Passage]:
